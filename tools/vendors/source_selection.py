"""Typed source-selection vendoring models and actionizer."""
...
class VendoringMode(StrEnum):
    """Closed operation vocabulary for source-selection vendoring."""

    CHECK = "check"
    SYNC = "sync"


class VendoringDisposition(StrEnum):
    """Closed successful outcomes for source-selection vendoring."""

    SELECTION_MATCHES = "selection_matches"
    SELECTION_SYNCHRONIZED = "selection_synchronized"


@dataclass(frozen=True, slots=True)
class VendorSourceSelectionRequest(DataObjectRequest):
    """Explicit inputs for one source-selection vendoring invocation."""
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.mode, VendoringMode):
            raise VendoringError("mode must be a VendoringMode")


@dataclass(frozen=True, slots=True)
class VendorSourceSelectionResponse(DataObjectResult):
    """Immutable successful response for one vendoring invocation."""

    request: VendorSourceSelectionRequest
    disposition: VendoringDisposition

    def __post_init__(self) -> None:
        expected = (
            VendoringDisposition.SELECTION_SYNCHRONIZED
            if self.request.mode is VendoringMode.SYNC
            else VendoringDisposition.SELECTION_MATCHES
        )
        if self.disposition is not expected:
            raise VendoringError("disposition contradicts the requested mode")


@dataclass(frozen=True, slots=True)
class FileSelection(DataObject):
    role: str
    source: str
    destination: str
...
@dataclass(frozen=True, slots=True)
class MaintainedFile(DataObject):
    role: str
    destination: str
    origin: str
...
@dataclass(frozen=True, slots=True)
class VendorSourceSelectionActionizer(
    DataObjectActionizer[
        VendorSourceSelectionRequest,
        VendorSourceSelectionResponse,
    ]
):
    """Check or synchronize one provenance-authorized source selection."""
    ...
    def actionize(
        ...
        """Perform the explicit vendoring mode and return its closed response."""

        _vendor(
            repository_root=request.repository_root,
            recipe_path=request.recipe_path,
            checkout=request.checkout,
            sync=request.mode is VendoringMode.SYNC,
        )
        disposition = (
            VendoringDisposition.SELECTION_SYNCHRONIZED
            if request.mode is VendoringMode.SYNC
            else VendoringDisposition.SELECTION_MATCHES
        )
        return VendorSourceSelectionResponse(
            request=request,
